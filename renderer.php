...
class block_tb_my_courses_renderer extends plugin_renderer_base {
    ...
    /**
     * Construct contents of tb_my_courses block
     *
     * @param array $courses list of courses in sorted order
     * @param stdClass|stdObject $config settings from Leeloo
     * @return string html to be displayed in tb_my_courses block
     */
    public function tb_my_courses($courses, $config) {
        global $CFG, $DB;
        $html = '';
        // LearningWorks.
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_tb_my_courses.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_tb_my_courses', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/tb_my_courses/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                array('src' => $moveicon,
                    'alt' => get_string('movetofirst', 'block_tb_my_courses', $courses[$movingcourseid]->fullname),
        ...
        foreach ($courses as $key => $course) {
            ...
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                    array('src' => $moveicon->out(false),
                        'alt' => get_string('movecourse', 'block_tb_my_courses', $course->fullname),
            ...
            if (!empty($config->mycourse_showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_tb_my_courses_get_child_shortnames($course->id)) {
            ...
            if ($course->id > 0) {
                ...
                $html .= block_tb_my_courses_build_progress($course, $config);
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                    array('src' => $movehere,
                        'alt' => get_string('moveafterhere', 'block_tb_my_courses', $a),
                        'title' => get_string('movehere')));
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_tb_my_courses', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_tb_my_courses'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumber', $options, block_tb_my_courses_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_tb_my_courses'));
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_tb_my_courses.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_tb_my_courses', $USER->firstname));

        if (!empty($CFG->messaging)) {
            $plural = 's';
            if ($msgcount > 0) {
                $output .= get_string('youhavemessages', 'block_tb_my_courses', $msgcount);
                if ($msgcount == 1) {
                    $plural = '';
                }
            } else {
                $output .= get_string('youhavenomessages', 'block_tb_my_courses');
            }
            $output .= html_writer::link(new moodle_url('/message/index.php'),
                get_string('message' . $plural, 'block_tb_my_courses'));
        }
        $output .= $this->output->box_end();
    ...
    public function course_image($course, $config) {
        ...
        if ($course->id > 0 && $course->get_course_overviewfiles()) {
            foreach ($course->get_course_overviewfiles() as $file) {
                ...
                if ($isimage) {
                    if (is_null($config->mycourse_tb_a_courses_bgimage) ||
                        $config->mycourse_tb_a_courses_bgimage == BLOCKS_TB_MY_COURSES_IMAGEASBACKGROUND_FALSE) {
    ...
    public function course_image_defaults($config) {
        ...
        if (method_exists($this->output, 'image_url')) {
            // Use the new method.
            $default = $this->output->image_url('default', 'block_tb_my_courses');
        } else {
            // Still a pre Moodle 3.3 release. Use pix_url because image_url doesn't exist yet.
        ...
        if ($courseimagedefault = @$config->mycourse_courseimagedefault) {
            ...
            // We check for a default image in the tb_my_courses pix folder named default aka our final hope.
            $imageurl = $default;
        }
